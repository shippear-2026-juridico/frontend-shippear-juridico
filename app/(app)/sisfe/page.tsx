...
import { AlertCircle, Cable, ChevronLeft, ChevronRight, Clock3, Download, RefreshCw, Search, ShieldCheck } from "lucide-react"
...
export default function SisfePage() {
  ...
  useEffect(() => {
    ...
    if (connection === "success") {
      toast.success(`${params.get("count") ?? "Los"} expedientes fueron actualizados desde SISFE`)
      void Promise.all([
  ...
  return (
    <div className="mx-auto max-w-[1500px]">
      ...
      <Card><CardContent className="p-0">
        ...
        <div className="hidden overflow-x-auto md:block"><Table><TableHeader><TableRow><TableHead>Expediente</TableHead><TableHead>Carátula</TableHead><TableHead>Inicio</TableHead><TableHead>Actualización SISFE</TableHead><TableHead>Radicación actual</TableHead><TableHead>Novedades</TableHead></TableRow></TableHeader><TableBody>
          {expedientes.isLoading ? Array.from({ length: 5 }).map((_, index) => <TableRow key={index}><TableCell colSpan={6}><Skeleton className="h-10 w-full" /></TableCell></TableRow>) : expedientes.data?.items.length ? expedientes.data.items.map((item) => <TableRow key={item.id}><TableCell className="font-mono text-xs"><Link href={`/sisfe/${item.id}`} className="font-medium text-emerald-700 hover:underline">{item.cuij || item.numero}</Link></TableCell><TableCell className="max-w-md"><Link href={`/sisfe/${item.id}`} className="block line-clamp-2 font-medium">{item.caratula}</Link></TableCell><TableCell>{date(item.fechaInicio)}</TableCell><TableCell>{date(item.fechaActualizacion)}</TableCell><TableCell className="max-w-sm"><p className="line-clamp-2">{item.radicacion || "Sin radicación informada"}</p><p className="mt-1 text-xs text-muted-foreground">{item.localidad || "Rosario"}</p></TableCell><TableCell><Badge variant="secondary">{item._count?.movements ?? 0}</Badge></TableCell></TableRow>) : <TableRow><TableCell colSpan={6} className="h-48 text-center text-muted-foreground">No hay expedientes sincronizados que coincidan.</TableCell></TableRow>}
        </TableBody></Table></div>
        <div className="grid gap-3 p-4 md:hidden">{expedientes.isLoading ? <Skeleton className="h-36" /> : expedientes.data?.items.map((item) => <Link key={item.id} href={`/sisfe/${item.id}`} className="rounded-xl border bg-background p-4"><p className="font-mono text-xs text-emerald-700">{item.cuij || item.numero}</p><p className="mt-2 line-clamp-2 text-sm font-medium">{item.caratula}</p><p className="mt-3 text-xs text-muted-foreground">{item.radicacion}</p><div className="mt-3 flex items-center justify-between text-xs text-muted-foreground"><span>Actualizado {date(item.fechaActualizacion)}</span><Badge variant="secondary">{item._count?.movements ?? 0} novedades</Badge></div></Link>)}</div>
