...
export default function SisfeDetailPage() {
  ...
  const officialUrl = (document: SisfeDocument) => {
    ...
    return document.source === "CARGO" && movementSisfeId
      ? `https://sisfe.justiciasantafe.gov.ar/documentos-adjuntos/${movementSisfeId}/${item.sisfeId}`
      : `https://sisfe.justiciasantafe.gov.ar/detalle-expediente/${item.sisfeId}`
  }
      ...
      <div className="min-w-0 space-y-6">
        <Card className="min-w-0 overflow-hidden">
          ...
          <CardContent className="min-w-0">
            ...
            </div>)}</div> : <p className="py-8 text-center text-sm text-muted-foreground">Todavía no se registraron adjuntos. Ejecutá nuevamente “Conectar y actualizar” con la extensión 0.8.0.</p>}
          </CardContent>
        </Card>
